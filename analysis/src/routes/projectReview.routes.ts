...
router.route('/').post(isauthenticated,val.create , controllers.createProjectReviewHandler)
  .get(val.getAll , globalPaginationMiddleware , controllers.getProjectsPagination , controllers.getProjectReviewsHandler);

router.route('/:reviewId').patch(isauthenticated , val.update , controllers.updateProjectReviewHandler)
