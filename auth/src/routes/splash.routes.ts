import { BadRequestError, Bucket, FOLDERS, globalUploadMiddleware, Setting } from '@duvdu-v1/duvdu';
import {Router} from 'express';

const router = Router();

router.post('/', globalUploadMiddleware('setting' ).single('cover'), async (req , res, next ) => {
...
router.delete('/:id', async (req , res ) => {
  ...
  const splashIndex = setting.splash.findIndex((item) => item._id?.toString() === id);
  if (splashIndex === -1) {
