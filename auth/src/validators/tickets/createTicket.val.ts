...
export const createTicketVal = [
  ...
  check('phoneNumber.key').not().exists().withMessage('phoneNumberKeyNotAllowed').customSanitizer(() => '+2'),
  check('phoneNumber.number').exists().isString().isMobilePhone('ar-EG').withMessage('phoneNumberInvalid'),
