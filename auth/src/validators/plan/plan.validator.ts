...
export const create = [
  body('key').exists().isString().trim().withMessage('keyString').isLength({ min: 2 }).withMessage('keyLength'),
  body('title').exists().isString().trim().withMessage('titleString').isLength({ min: 2 }).withMessage('titleLength'),
