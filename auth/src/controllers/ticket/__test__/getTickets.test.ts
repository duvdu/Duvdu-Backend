...
describe('getTickets should ' , ()=>{
  it('return 401 if user unauthenticated' , async ()=>{
    await request.get('/api/users/ticket').expect(401);
  });
  it('return 403 if user dont have permission' , async ()=>{
    await request.get('/api/users/ticket')
      .set('Cookie' , cookieSession)
      .expect(403);
  });

  it('return 200 for success' , async ()=>{
    await mongoose.connection.db.collection('role').updateOne({_id: new Types.ObjectId(mongoId)} , { $set: { features: [Ifeatures.getAllTickets] } });    
    await request.get('/api/users/ticket')
      .set('Cookie' , cookieSession)
      .expect(200);
  });
