...
describe('update ticket should ' , ()=>{
  it('return 401 if user unauthenticate' , async ()=>{
    await request.put(`/api/users/ticket/${id}`).expect(401);
  });
  it('return 403 if user dont have this permission' , async ()=>{
    await request.put(`/api/users/ticket/${id}`).set('Cookie',cookieSession).expect(403);
  });
  it('return 422 if for invalid input' , async ()=>{
    await mongoose.connection.db.collection('role').updateOne({_id: new Types.ObjectId(mongoId)} , { $set: { features: [Ifeatures.updateTicket] } });    
    await request.put(`/api/users/ticket/${id}`).set('Cookie',cookieSession).expect(422);
  });
  it('return 422 if for invalid input' , async ()=>{
    await mongoose.connection.db.collection('role').updateOne({_id: new Types.ObjectId(mongoId)} , { $set: { features: [Ifeatures.updateTicket] } });    
    await request.put('/api/users/ticket/123').set('Cookie',cookieSession).expect(422);
  });
  it('return 200 for success' , async ()=>{
    ...
    await request.put(`/api/users/ticket/${response?._id}`)
      .set('Cookie',cookieSession)
      .send({
        state:{
