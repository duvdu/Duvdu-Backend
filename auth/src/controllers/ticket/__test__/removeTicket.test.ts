...
describe('remove ticket should be ', () => {
  it('return 401 if user unauthenticated', async () => {
    await request.delete('/api/users/tickets/123').expect(401);
  });

  it('return 403 if user dont have this permission', async () => {
    await request.delete('/api/users/tickets/123').set('Cookie', cookieSession).expect(403);
  });
  it('return 422 for invalid id format ', async () => {
    await mongoose.connection.db
      ...
      .updateOne(
        ...
        { $set: { features: [Ifeatures.removeTicket] } },
      );
    await request.delete('/api/users/tickets/123').set('Cookie', cookieSession).expect(422);
  });
  it('return 404 if ticket not found ', async () => {
    ...
    await request.delete(`/api/users/tickets/${id}`).set('Cookie', cookieSession).expect(404);
  });
  it('return 204 for success', async () => {
    ...
    await request
      .delete(`/api/users/tickets/${response?._id}`)
      .set('Cookie', cookieSession)
      .expect(204);
  });
});
