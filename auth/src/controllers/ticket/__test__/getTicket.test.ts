...
describe('getTicket should ' , ()=>{
  it('return 401 if user unauthenticated' , async ()=>{
    await request.get('/api/users/ticket/234').expect(401);
  });
  it('return 403 if user dont have this permission' , async ()=>{
    await request.get('/api/users/ticket/234')
      .set('Cookie' , cookieSession)
      .expect(403);
  });
  it('return 422 for invalid id format' , async ()=>{
    await mongoose.connection.db.collection('role').updateOne({_id: new Types.ObjectId(mongoId)} , { $set: { features: [Ifeatures.getTicket] } });    
    await request.get('/api/users/ticket/234')
      .set('Cookie' , cookieSession)
      .expect(422);
  });
  it('return 404 if ticket not found' , async ()=>{
    await mongoose.connection.db.collection('role').updateOne({_id: new Types.ObjectId(mongoId)} , { $set: { features: [Ifeatures.getTicket] } });    
    await request.get(`/api/users/ticket/${mongoId}`)
      .set('Cookie' , cookieSession)
      .expect(404);
  });
  it('return 200 for success' , async ()=>{
    ...
    await request.get(`/api/users/ticket/${response?._id}`)
      .set('Cookie' , cookieSession)
      .expect(200);
  });
});