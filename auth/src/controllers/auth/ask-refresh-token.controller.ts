...
export const askRefreshTokenHandler: RequestHandler = async (req, res, next) => {
  ...
  const refreshToken = clientType === 'web' ? req.session.refresh : req.session.mobileRefresh;

  if (!refreshToken) {
    return next(new UnauthenticatedError({ en: 'refresh token not found', ar: 'الرمز المميز لإعادة التحميل غير موجود' }, req.lang));
  }

  let payload: { id: string };
  try {
    payload = <{ id: string }>verify(refreshToken, env.jwt.secret);
    const user = await Users.findById(payload.id).populate('role');
    if (!user) {
    ...
    if (clientType === 'web')
      ...
      req.session.mobileAccess = token;
