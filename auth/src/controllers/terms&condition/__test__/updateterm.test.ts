...
describe('update term should ', () => {
  ...
  it('return 403 if user dont have permission', async () => {
    await request.put('/api/users/terms/123').set('Cookie', cookieSession).expect(403);
  });
  it('return 422 for invalid input', async () => {
    await mongoose.connection.db
      ...
      .updateOne(
        ...
        { $set: { features: [Ifeatures.updateTerm] } },
      );
    await request.put('/api/users/terms/123').set('Cookie', cookieSession).expect(422);
  });
  it('return 422 for invalid input', async () => {
    await mongoose.connection.db
      .collection('role')
  ...
  it('return 200 for success', async () => {
    await mongoose.connection.db
      ...
      .updateOne(
        { _id: new Types.ObjectId(mongoId) },
        { $set: { features: [Ifeatures.updateTerm, Ifeatures.createTerm] } },
      );
    await request
    ...
    const response = await mongoose.connection.db
      .collection('terms')
      .findOne({ desc: 'sadasdasd' });

    await request
      .put(`/api/users/terms/${response?._id}`)
      .set('Cookie', cookieSession)
      .send({
        desc: 'asdasdadadad',
      })
