...
const projectIds = {
  ...
  p4: new mongoose.Types.ObjectId().toHexString(),
  p5: new mongoose.Types.ObjectId().toHexString(),
  p6: new mongoose.Types.ObjectId().toHexString(),
};
...
describe('get project lists controller', () => {
  it('should return 200 with populated first 3 projects', async () => {
    const response = await request
    ...
    expect(response.body.data?.[0].projects.length).toBe(3);
    expect(response.body.data?.[0].projects?.[0].title).toBeDefined();
    expect(response.body.data?.[0].projects?.[1].title).toBeDefined();
    expect(response.body.data?.[0].projects?.[2].title).toBeDefined();
  });
});
