...
export async function filterTagsForCategory(categoryId: string, subcategoryId: string, tagIds: string[], cycle: string): Promise<{ subCategoryTitle: Ititle; filteredTags: Itag[] }> {
  const category = await Categories.findOne({ _id: categoryId });
  if (!category) throw new NotFound('Category not found');
  if (category.cycle !== cycle) throw new BadRequestError('This category is not related to this cycle');
  ...
  if (filteredTags.length !== tagIds.length) throw new BadRequestError('Invalid tags');
