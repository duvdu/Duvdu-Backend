...
interface IdsWithUnits {
  tools?: Array<{ id: string; units?: number; unitPrice?: number }>;
  functions?: Array<{ id: string; units?: number; unitPrice?: number }>;
}
...
export async function calculateTotalPrice(
  ...
  if (idsWithUnits.tools) {
    for (const tool of idsWithUnits.tools) {
      ...
      const units = tool.units ?? 1;
      const unitPrice = tool.unitPrice ?? toolItem.unitPrice;
      const itemTotalPrice = unitPrice * units;
      totalPrice += itemTotalPrice;
      result.tools.push({
        _id: tool.id,
        name: toolItem.name,
  ...
  if (idsWithUnits.functions) {
    for (const func of idsWithUnits.functions) {
      ...
      const units = func.units ?? 1;
      const unitPrice = func.unitPrice ?? functionItem.unitPrice;
      const itemTotalPrice = unitPrice * units;
      totalPrice += itemTotalPrice;
      result.functions.push({
        _id: func.id,
