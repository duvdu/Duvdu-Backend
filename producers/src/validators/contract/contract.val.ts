...
export const createContractVal = [
  ...
  body('appointmentDate').isISO8601().withMessage('appointmentDate.invalid').custom((val) => {
...
export const updateContractVal = [
  ...
  body('appointmentDate').optional().isISO8601().withMessage('appointmentDate.invalid').custom((val, { req }) => {
    if (req.body.status) throw new Error('status cant provided with appointmentDate');
    if (new Date(val) > new Date()) return true;
    throw new Error('appointmentDate.past');
  }),
  body('status').optional().isIn(['canceled', 'accepted', 'rejected']).withMessage('status.invalid'),
  globalValidatorMiddleware
];
