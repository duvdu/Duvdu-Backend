...
export const deleteMessageHandler:DeleteMessageHandler = async (req,res,next)=>{
  ...
  if (!message) 
    return next(new NotFound({en:`message not found ${req.params.message}` , ar:`الرسالة غير موجودة ${req.params.message}`} , req.lang));

  if (message.sender.toString() != req.loggedUser.id) 
    return next(new UnauthorizedError({en:`user dont owner for this message ${req.params.message}` , ar:`المستخدم ليس مالكاً لهذه الرسالة ${req.params.message}`} , req.lang));
