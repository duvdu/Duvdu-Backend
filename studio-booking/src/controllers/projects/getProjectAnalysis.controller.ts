...
> = async (req, res) => {
  ...
  const totalCount = await studioBooking.countDocuments(matchedPeriod);
  ...
  const topUsers = await studioBooking.aggregate(topUsersPipeline);
  ...
  const addressStats = await studioBooking.aggregate(topAddressesPipeline);
  ...
  const priceStats = await studioBooking.aggregate(priceStatsPipeline);
  const totalPrice = priceStats.length > 0 ? priceStats[0].totalPrices : 0;

  const showOnHomeCount = await studioBooking.countDocuments({
    showOnHome: true,
    ...matchedPeriod,
  });

  const deletedBookingsCount = await studioBooking.countDocuments({
    isDeleted: true,
    ...matchedPeriod,
  });
