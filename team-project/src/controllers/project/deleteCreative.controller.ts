...
import { Bucket, NotAllowedError, NotFound } from '@duvdu-v1/duvdu';

import { ContractStatus, TeamContract, TeamProject } from '../../models/teamProject.model';
...
export const deleteCreativeHandler:DeleteCreativeHandler = async (req,res,next)=>{
  ...
  if (!(contract.status != ContractStatus.pending  && contract.status != ContractStatus.rejected && contract.status != ContractStatus.canceled) ) 
    return next(new NotAllowedError({en:'can not delete this creative invalid contract status' , ar:'لا يمكن حذف حالة العقد الإبداعي غير الصالح'} , req.lang));

  const index = team.creatives.findIndex(creative => creative.category.toString() === req.body.category);

  if (index === -1)
    return next(new NotFound({en:'category not found in team' , ar:'الفئة غير موجودة في الفريق'} , req.lang));

  const userIndex = team.creatives[index].users.findIndex(user => user.user.toString() === req.body.user);
  if (userIndex === -1) 
    return next(new NotFound({en:'user not include in this category' , ar:'لم يتم تضمين المستخدم في هذه الفئة'} , req.lang));
