...
describe('update category should be' , ()=>{
  it('should return 422 for invalid input' , async ()=>{
    await request.put(`/api/category/${id}`)
      .field('title.ar' , '')
  ...
  it('should return 200 for success ' , async ()=>{
    await request.post('/api/category')
      ...
      .set('Content-Type', 'multipart/form-data')
      .expect(201);
    const category = await mongoose.connection.db.collection('categories').findOne({title:{en:'category' , ar:'ةةةةةةة'}});
    await request.put(`/api/category/${category?._id}`)
      .field('title.en' , 'category')
      .field('title.ar' , 'معتمد')
      .attach('image', fs.readFileSync(path.join(__dirname, 'image.jpg')), 'image.jpg')
      .field('jobTitles[0]' ,'updated')
