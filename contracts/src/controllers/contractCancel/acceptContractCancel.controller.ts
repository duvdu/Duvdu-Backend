...
> = async (req, res) => {
  ...
  if (!contractCancel)
    throw new NotFound({ ar: 'الطلب غير موجود', en: 'Contract cancel not found' }, req.lang);
  ...
  if (contract.ref === MODELS.projectContract) {
    ...
    await RentalContracts.findOneAndUpdate(contractCancel.contract, {
      status: RentalContractStatus.canceled,
    });
  } else if (contract.ref === MODELS.copyrightContract) {
    await CopyrightContracts.findOneAndUpdate(contractCancel.contract, {
      status: CopyrightContractStatus.canceled,
    });
  } else if (contract.ref === MODELS.producerContract) {
    await ProducerContract.findOneAndUpdate(contractCancel.contract, {
