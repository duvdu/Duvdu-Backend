...
> = async (req, res, next) => {
  ...
  if (coupon.expired)
    return next(
      new BadRequestError({ en: 'coupon expired', ar: 'انتهت صلاحية القسيمة' }, req.lang),
    );

  if (!(new Date(coupon.start) >= new Date()))
    return next(
      new BadRequestError({
        en: `coupon not start yet can use it in ${coupon.start}`,
        ar: `لم تبدأ القسيمة بعد، يمكنك استخدامها في ${coupon.start}`,
      }),
    );
  ...
  if (coupon.couponCountAvaliable === 0)
    return next(
      new BadRequestError(
        {
          en: 'The coupon has exceeded the maximum usage limit.',
          ar: 'القسيمة تعدت الحد الاقصى من الاستخدام',
        },
        req.lang,
      ),
    );
  ...
  if (!user) coupon.users.push({ user: new mongoose.Types.ObjectId(req.loggedUser.id), count: 1 });
